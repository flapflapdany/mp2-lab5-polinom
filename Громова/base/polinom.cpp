...
{
	string kk;
	for (auto it = p.monom.Start(); it != p.monom.End(); ++it)
	{
		int deg = it->data.GetPow();
		if (it != p.monom.Start() && it->data.GetCoeff() > 0)
		{
			kk += "+";
		}
...
Polinom Polinom::operator+(Polinom& p)
{
	...
	List<Monom>::LinkPointer it1, it2;
...
ostream & operator<<(ostream & os, Polinom & p)
{
	...
	{
		int deg = it->data.GetPow();
		if (it != p.monom.Start() && it->data.GetCoeff() > 0.0)
		{
			os << "+";
